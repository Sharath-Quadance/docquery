...
@validate_arguments
def load_document(fpath: str, ocr_reader: Optional[Union[str, OCRReader]] = None, use_embedded_text=True,subscription_key = None,endpoint = None):
    ...
    if not ocr_reader or isinstance(ocr_reader, str):
        if ocr_reader == "azureocr":
            ocr_reader = get_ocr_reader(ocr_reader,subscription_key=subscription_key,endpoint=endpoint)
        else:
